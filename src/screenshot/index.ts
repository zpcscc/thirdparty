...
import nonNegativeData from 'src/screenshot/lib/common-methods/NonNegativeData';
import CreateDom from 'src/screenshot/lib/main-entrance/CreateDom';
...
import {
  ...
  startCapture,
  stopCapture
} from 'src/screenshot/lib/main-entrance/LoadCoreComponents';
import { mouseDownCore, mouseUpCore } from 'src/screenshot/lib/main-entrance/MouseEventHandling';
...
import getDrawBoundaryStatus from 'src/screenshot/lib/split-methods/BoundaryJudgment';
import drawCutOutBox from 'src/screenshot/lib/split-methods/DrawCutOutBox';
import drawMasking from 'src/screenshot/lib/split-methods/DrawMasking';
import KeyboardEventHandle from 'src/screenshot/lib/split-methods/KeyboardEventHandle';
import setPlugInParameters from 'src/screenshot/lib/split-methods/SetPlugInParameters';
import type {
  crcEventType,
  cutOutBoxBorder,
...
import './assets/screen-shot.scss';
...
export default class ScreenShot {
  ...
  private screenShotDom: HTMLElement | HTMLDivElement | HTMLCanvasElement | null = null;
  // 截图工具栏dom
  ...
  constructor(options: screenShotType) {
    ...
    this.load(triggerCallback, cancelCallback).then(() => {
      ...
      // 为截图容器创建事件监听
      this.createEvent(this.screenShotContainer, this.toolController, this.textInputController);
    });
  }
  ...
  // 加载截图组件
  private async load(triggerCallback?: AnyFunction, cancelCallback?: AnyFunction) {
    const canvasSize = this.plugInParameters.getCanvasSize();
    const viewSize = {
      width: Number.parseFloat(window.getComputedStyle(document.body).width),
      height: Number.parseFloat(window.getComputedStyle(document.body).height)
    };
    // 设置截图容器尺寸信息
    setScreenShotContainerSize(
    ...
    if (!this.plugInParameters.getWebRtcStatus()) {
      ...
      if (this.imgSrc != null) {
        ...
        drawPictures(this.imgSrc, this.screenShotImageController, callerCallback);
        return;
      }
      const result = await h2cScreenShot(triggerCallback, this.screenShotDom);
      if (result.code === 0) {
        // 存储html-to-image截取的内容
  ...
  private loadScreenFlowData(triggerCallback: AnyFunction | undefined) {
    setTimeout(() => {
      // 获取截图区域canvas容器画布
      if (this.screenShotContainer == null) return;
      const { containerWidth, containerHeight, imgContainerWidth, imgContainerHeight } =
        getContainerSize(
      ...
      if (this.captureStream) {
        // 获取当前选择的窗口类型
        displaySurface = this.captureStream.getVideoTracks()[0].getSettings()?.displaySurface ?? '';
        // 获取当前选择的标签页标识
  ...
  /**
   * 初始化截图容器
   * src/screenshotparam triggerCallback
   * src/screenshotparam context
   * src/screenshotparam screenShotContainer
   * src/screenshotprivate
   */
  ...
  private mouseUpEvent = () => {
    ...
    const updateTempGraphPositionCallback = (res: genericMethodPostbackType) => {
      if (res.code === 1 && res.data != null) {
  ...
  private setOptionalParameter(options: screenShotType) {
    ...
    if (options?.hiddenScrollBar != null) {
      ...
      this.hiddenScrollBar = {
        state,
        color: color || '#000000',
        fillWidth: fillWidth || 0,
        fillHeight: fillHeight || 0,
        fillState: fillState || false
      };
      if (state) {
  ...
  private createEvent(
    ...
    // 创建键盘事件监听
    this.keyboardEventHandle = new KeyboardEventHandle(screenShotContainer, toolController);
    // 给输入容器设置快捷键监听
    registerContainerShortcuts(
      textInputController,
      this.screenShotCanvas,
