import getToolRelativePosition from 'src/screenshot/lib/common-methods/GetToolRelativePosition';
...
import type { positionInfoType, textInfoType } from 'src/screenshot/lib/type/ComponentType';
...
export default class InitData {
  ...
  public getScreenShotContainer() {
    screenShotController = document.querySelector(
      '#screenShotContainer'
  ...
  public getToolController() {
    toolController = document.querySelector('#toolPanel') as HTMLDivElement | null;
    return toolController;
  }

  // 获取裁剪框尺寸显示容器
  public getCutBoxSizeContainer() {
    cutBoxSizeContainer = document.querySelector('#cutBoxSizePanel') as HTMLDivElement | null;
    return cutBoxSizeContainer;
  }

  // 获取文本输入区域dom
  public getTextInputController() {
    textInputController = document.querySelector('#textInputPanel') as HTMLDivElement | null;
  ...
  public setCutBoxSizePosition(x: number, y: number) {
    ...
    if (screenShotController) {
      sscTop = Number.parseInt(screenShotController.style.top);
    }
  ...
  public setCutBoxSize(width: number, height: number) {
    ...
    if (childrenPanel.length > 0) {
      (childrenPanel[0] as HTMLParagraphElement).textContent = `${floorWidth} * ${floorHeight}`;
      return;
    }
    // 不存在则渲染
    const textPanel = document.createElement('p');
    textPanel.textContent = `${floorWidth} * ${floorHeight}`;
    cutBoxSizeContainer.append(textPanel);
  }
  ...
  public setToolInfo(left: number, top: number) {
    toolController = document.querySelector('#toolPanel') as HTMLDivElement;
    const { left: rLeft, top: rTop } = getToolRelativePosition(left, top);
  ...
  // 设置裁剪框位置信息
  public setCutOutBoxPosition(mouseX: number, mouseY: number, width: number, height: number) {
  ...
  public getOptionIcoController() {
    optionIcoController = document.querySelector('#optionIcoController') as HTMLDivElement | null;
    return optionIcoController;
  }

  public getTextSizeContainer() {
    textSizeContainer = document.querySelector('#textSizePanel') as HTMLDivElement | null;
    return textSizeContainer;
  }

  public getOptionTextSizeController() {
    optionTextSizeController = document.querySelector('#textSelectPanel') as HTMLDivElement | null;
    return optionTextSizeController;
  }

  public getBrushSelectionController() {
    brushSelectionController = document.querySelector('#brushSelectPanel') as HTMLDivElement | null;
    return brushSelectionController;
  }

  public getOptionController() {
    optionController = document.querySelector('#optionPanel') as HTMLDivElement | null;
  ...
  public getColorSelectPanel() {
    colorSelectPanel = document.querySelector('#colorSelectPanel');
  ...
  public getColorPanel() {
    colorSelectController = document.querySelector('#colorPanel');
  ...
  public getUndoController() {
    undoController = document.querySelector('#undoPanel');
  ...
  public destroyDOM() {
    ...
    optionIcoController.remove();
    optionController.remove();
    textInputController.remove();
    cutBoxSizeContainer.remove();
