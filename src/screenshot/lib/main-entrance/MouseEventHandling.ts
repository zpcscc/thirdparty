...
const mouseDownCore = (
  ...
  if (!isPC() && event instanceof TouchEvent && containerInfo.screenShotCanvas) {
    operatingCutOutBox(
...
const mouseUpCore = (
  ...
  if (
    ...
    // 复原裁剪框的坐标
    containerVariable.drawGraphPosition.startX = containerVariable.drawGraphPrevX;
    containerVariable.drawGraphPosition.startY = containerVariable.drawGraphPrevY;
    return;
  }
  ...
  if (containerInfo.screenShotContainer == null || containerInfo.screenShotCanvas == null) {
    return;
  }
  // 工具栏已点击且进行了绘制
  ...
  // 保存绘制后的图形位置信息
  Object.assign(containerVariable.drawGraphPosition, containerVariable.tempGraphPosition);
  // 如果工具栏未点击则保存裁剪框位置
