export default class DrawArrow {
  ...
  // 计算箭头底边两个点位置信息
  private arrowCord(beginPoint: { x: number; y: number }, stopPoint: { x: number; y: number }) {
    ...
    this.polygonVertex[8] =
      stopPoint.x -
      this.arrowInfo.edgeLen * Math.cos((Math.PI / 180) * (this.angle + this.arrowInfo.angle));
    this.polygonVertex[9] =
      stopPoint.y -
      this.arrowInfo.edgeLen * Math.sin((Math.PI / 180) * (this.angle + this.arrowInfo.angle));
    // 使用三角函数计算出4、5顶点的坐标
  ...
  // 计算两个点之间的夹角
  private getRadian(beginPoint: { x: number; y: number }, stopPoint: { x: number; y: number }) {
    // 使用atan2算出夹角（弧度），并将其转换为角度值(弧度 / 180)
    this.angle =
      (Math.atan2(stopPoint.y - beginPoint.y, stopPoint.x - beginPoint.x) / Math.PI) * 180;
