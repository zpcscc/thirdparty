...
const drawCutOutBox = (
  ...
  const width = widthValue === 0 ? 5 : widthValue;
  const height = heightValue === 0 ? 5 : heightValue;
  ...
  let { imgWidth, imgHeight } = {
    imgWidth: Number.parseInt(controller?.style.width),
    imgHeight: Number.parseInt(controller?.style.height)
  };
